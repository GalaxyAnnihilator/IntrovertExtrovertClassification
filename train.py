...
sns.heatmap(
    df[numeric_columns].corr(), annot=True, cmap="coolwarm", fmt=".2f", vmin=-1, vmax=1
)
...
report_dict = classification_report(
    y_test, predictions, labels=pipe.classes_, output_dict=True
)
...
encoder = (
    pipe.named_steps["preprocessing"].named_transformers_["cat"].named_steps["encoder"]
)
...
feature_importance = pd.DataFrame(
    {"Feature": final_features, "Importance": importances}
...
sns.barplot(
    ...
    palette="viridis",
    hue=feature_importance["Importance"],
)
